...
// World Triathlon Corporaion (WTC).
...
function iTri(s){
    const value = (140.6 - s).toFixed(2) + ' to go!'
    if (s === 0) return 'Starting Line... Good Luck!';
    else if (s < 2.4) return {Swim: value};
    else if (s < 114.4) return {Bike: value};
    else if (s < 130.6) return {Run: value};
    else if (s < 140.6) return {Run: 'Nearly there!'};
    else return 'You\'re done! Stop running!'
};

console.log(iTri(1.6));
